...
export class GuestReservationsComponent implements OnInit {
  ...
  sendNotificationAfterCancelation(reservationId: string): void {
    ...
    if (token) {
      ...
      const notificationData = {
        host_id: decodedToken.userID, // Assuming the host ID should be used here; adjust as necessary
        message: message
      };
  ...
  toggleHostRatingForm(reservationId: string | undefined): void {
    ...
    this.loadHostRating(reservationId); // Učitavanje ocene hosta
  }
  ...
  toggleAccommodationRatingForm(reservationId: string | undefined): void {
    ...
    this.showEditRatingForm = false;
    this.selectedReservationId = reservationId;
    this.loadAccommodationRating(reservationId); // Učitavanje ocene smeštaja
  }
  ...
  sendNotification(message: string): void {
    ...
    if (token) {
      const decodedToken = this.decodeToken(token);
      const notificationData = {
        host_id: decodedToken.userID, // Koristimo ID korisnika iz tokena
        message: message
      };
  ...
  submitEditRating(reservationId: string): void {
    // Implementacija za čuvanje izmenjenih ocena
