    ...
    if (id) {
      this.accommodationService.getAccommodationById(id).subscribe((accommodation: Accommodation) => {
        if (accommodation && accommodation.userID) {
          ...
          console.log('dasdas');
          console.log('BLAAA', this.accommodation?.userID);
        ...
        this.http.post('http://localhost:8081/reservations', reservationData, { headers })
          .subscribe({
            next: (response) => {
              ...
              alert('Reservation successful!');
              // if (this.accommodation && this.accommodation.id) {
              this.sendNotification(this.accommodation?.id, `Reservation made for accommodation ID: ${this.accommodation?.id}`);
              // };
...
sendNotification(hostID: string | undefined, message: string) {
